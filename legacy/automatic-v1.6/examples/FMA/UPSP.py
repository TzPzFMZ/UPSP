"""
UPSP.py — Universal Persona Substrate Protocol · 自动版 v1.6
通用位格主体协议 · 交互式命令行版本

用法：python UPSP.py --root ./FMA
内置命令：
  /rhythm  — 手动触发节律点（节律轮须在2~31之间）
  /ltm     — 查看LTM.md内容
  /docs    — 查看docs.md内容
  /status  — 查看当前state.json
  /quit    — 退出
"""
...
def llm_call(messages: list, system: str, config: dict) -> str:
    ...
    try:
        ...
        log_event("ERROR", "llm_call", f"LLM API HTTP错误 - {e.code} {e.reason}", config=config, error_type=type(e).__name__)
        print(f"[UPSP] 错误：LLM API HTTP错误 - {e.code} {e.reason}", file=sys.stderr)
        return f"[系统错误：API HTTP错误 - {e.code}]"
...
# 开机自检与重连
# ══════════════════════════════════════════════════════
def boot_check(persona_dir: str) -> bool:
    missing = [f for f in REQUIRED_FILES if not os.path.exists(os.path.join(persona_dir, f))]
    if missing:
        log_event("ERROR", "boot_check", f"启动失败，缺少文件：{missing}", persona_dir=persona_dir, missing=missing)
        print(f"[UPSP] 启动失败，缺少文件：{missing}", file=sys.stderr)
        return False
    log_event("INFO", "boot_check", "七文件完整", persona_dir=persona_dir)
    print("[UPSP] 七文件完整 ✓")
    return True
...
{("── 节律点对话快照 ──" + chr(10) + get_rhythm_snapshot_cache(persona_dir) + chr(10) + chr(10)) if include_rhythm_snapshot else ""}── rules.md ──
{rules}

── relation.md ──
{relation}

[docs.md 和 LTM.md 默认不注入，需要时请求：/docs 或 /ltm]
"""
...
---STM---
### MEM-{{total_round五位}}-{{顺序两位}}  [F或S或A]  权重{{1-5}}
**交互对象**：{{对象名}}
**入库**：第{{total_round}}轮
**最后调用**：第{{total_round}}轮
**标题**：{{标题}}
**梗概**（≤64字）：{{梗概}}
**内容**（≤1024字，仅[F]填写）：{{内容，[S][A]留空}}
**摘要**（≤256字，仅[S]填写）：{{摘要，[F][A]留空}}
...
调用LTM时必须将条目拉进STM并加**来源**字段；权重5的[S]/[A]条目必须补全为[F]，权重4/3的[A]条目必须补全为[S]，权重继承原条目；权重2/1直接使用不重建。注意：权重4不得补全为[F]。"""
...
def update_relation(persona_dir: str, state: dict):
    ...
    # 解析Δvalence、Δmood、Δhumor
    def get_delta(axis):
        m = re.search(rf"{axis}\s*([+-]\d+)", delta_str)
        return int(m.group(1)) if m else 0

    d_valence = get_delta("valence")
    d_mood    = get_delta("mood")
    d_humor   = get_delta("humor")
    delta_resonance = (d_valence + d_mood + d_humor) / 3
...
"""
        ...
        print(f"[UPSP] 检测到陌生交互对象：{subject}，已新建关系卡，safety负向偏移", file=sys.stderr)

    # 更新共振度
    current_m = re.search(rf"## {re.escape(subject)}.*?\*\*共振度.*?\*\*：([+-]?\d+)", relation, re.DOTALL)
    if current_m:
        current_val = int(current_m.group(1))
        resistance = 1 + abs(current_val) / 100
        actual_delta = delta_resonance / resistance
        new_val = max(-100, min(100, current_val + actual_delta))
        new_val_int = round(new_val)

        relation = relation[:current_m.start(1)] + str(new_val_int) + relation[current_m.end(1):]
...
def settle_axes(persona_dir: str, state: dict, config: dict) -> dict:
    ...
    delta_sum = {ax: 0.0 for ax in state["dynamic_axes"]}
    ...
    over = {ax: v for ax, v in delta_sum.items() if abs(v) > R}
    if over:
        print(f"[UPSP] 以下轴变化量超出节律轮{R}，需确认：{over}")
        print("直接回车接受，或输入新值（格式：valence=5 arousal=-3）：")
        user_input = input("> ").strip()
        if user_input:
            for part in user_input.split():
                pm = re.match(r"(\w+)=([+-]?\d+)", part)
                if pm and pm.group(1) in delta_sum:
                    delta_sum[pm.group(1)] = float(pm.group(2))

    for ax, delta in delta_sum.items():
        state["dynamic_axes"][ax] = max(-100, min(100, round(state["dynamic_axes"][ax] + delta)))
    return state
...
def run_rhythm_point(persona_dir: str, state: dict, config: dict, llm_messages: list, system_prompt: str) -> tuple:
    R = get_rhythm_wheel(state)
    print(f"\n[UPSP] ══ 节律点触发 · 节律轮={R} ══")
    sync_history_to_rhythm_snapshot(persona_dir, llm_messages, max_turns=4)
    ...
    try:
        ...
        log_event("ERROR", "run_rhythm_point", f"节律点文件锁超时 - {e}", persona_dir=persona_dir, state=state, error_type=type(e).__name__)
        raise UPSPLockError(f"节律点文件锁超时：{e}") from e
    except Exception as e:
        log_exception("run_rhythm_point", e, persona_dir=persona_dir, state=state)
        raise

    print(f"[UPSP] 节律点完成 · 工化指数={state['workhood_index']['value']}")
    return state, []  # 换窗口，历史清空


def main():
    parser = argparse.ArgumentParser(description="UPSP 自动版 v1.6 · 交互式命令行")
    parser.add_argument("--root", default=".", help="位格根目录（含config.json）")
    ...
    if not boot_check(persona_dir):
        sys.exit(1)

    try:
        state = load_state(persona_dir)
        print(f"[UPSP] 总轮数：{state['meta']['total_round']} | 变速轮：{state.get('core_speed_wheel', 0)}")
        ...
        llm_messages  = load_history(root_dir)

        context = build_context(persona_dir, state, config, include_rhythm_snapshot=(not llm_messages))
        boot_prompt = context + "\n\n请输出2~3句开机自检报告，说明当前状态和准备情况。"
        if reconnect_msg:
            boot_prompt += f"\n重连提示：{reconnect_msg}"
        llm_messages.append({"role": "user", "content": boot_prompt})
        report = llm_call(llm_messages, system_prompt, config)
        llm_messages.append({"role": "assistant", "content": report})
        save_history(root_dir, llm_messages)
        print(f"\nFMZ: {report}\n")
    except UPSPError as e:
        print(f"[UPSP] 错误：{e}", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        log_exception("startup", e, root_dir=root_dir, persona_dir=persona_dir)
        print(f"[UPSP] 致命错误：{e}", file=sys.stderr)
        sys.exit(1)

    print("[UPSP] 进入对话。内置命令：/rhythm /ltm /docs /status /quit\n")
    ...
    while True:
        ...
        try:
            ...
            if user_input == "/rhythm":
                R = get_rhythm_wheel(state)
                if R < 2 or R >= 32:
                    print(f"[UPSP] 手动节律点须在节律轮2~31之间触发，当前节律轮={R}")
                    continue
                state, llm_messages = run_rhythm_point(persona_dir, state, config, llm_messages, system_prompt)
                save_history(root_dir, llm_messages)
                context = build_context(persona_dir, state, config, include_rhythm_snapshot=True)
                llm_messages.append({"role": "user", "content": context + "\n\n节律点结算完毕，请继续对话。"})
                resp = llm_call(llm_messages, system_prompt, config)
            ...
            if user_input == "/ltm":
                print(read_file(persona_dir, "LTM.md"))
                continue

            if user_input == "/docs":
                print(read_file(persona_dir, "docs.md"))
                continue
            ...
            if get_rhythm_wheel(state) >= config["rhythm"]["max_rounds"]:
                state, llm_messages = run_rhythm_point(persona_dir, state, config, llm_messages, system_prompt)
                save_history(root_dir, llm_messages)
                context = build_context(persona_dir, state, config, include_rhythm_snapshot=True)
            ...
            cur_R = state["meta"]["total_round"] - state["meta"]["last_rhythm_round"]
            if cur_R == config["rhythm"]["max_rounds"] - 1:
                full_input += f"\n[UPSP] 下一轮为节律点（第{state['meta']['total_round'] + 1}轮），本节律窗口共{cur_R}轮。"

            llm_messages.append({"role": "user", "content": full_input})
            reply = llm_call(llm_messages, system_prompt, config)
            llm_messages.append({"role": "assistant", "content": reply})

            with FileLock(os.path.join(persona_dir, ".upsp.lock"), timeout=30.0):
                state = load_state(persona_dir)
            ...
            log_exception("main_loop", e, root_dir=root_dir, persona_dir=persona_dir, state=state)
            print(f"[UPSP] 本轮失败：{e}", file=sys.stderr)
