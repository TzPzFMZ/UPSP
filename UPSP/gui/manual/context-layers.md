...
分层机器真源是 `layers/*.json`，唯一实际发送体是 `step.json.request_body`，Round JSONL 保留调用与结算事件投影。页面首屏只读取 Frame 目录；用户选择当前或历史 Frame 后，才按需读取该 Frame 的十层正文与 manifest，并且前端只保留当前打开的一份详情。调用头与生成参数在“内容详情”中把合法 JSON 渲染为可递归展开的字段表格，并保留折叠的原始 JSON；工具头改用中文总览仪表显示权限、模式、终端工具、工具数量与传输状态，下面逐项列出工具，点击“查看详情”后在统一弹窗中显示真实 `description` 与参数 schema。其余层优先消费 `content_md`。详情读取失败只影响上下文面板并允许重试；渲染只改善阅读，不解析或改写 Runtime 事实。
