...
召回补全必须走 `memory_recall_complete`。它不是三表命中，也不是普通索引展开，而是当压缩正文、旧引用或链上证据不足以支撑当前判断时，对目标记忆正文进行一次受约束重写。提交 `memory_recall_completion_request` 时必须给出 `mem_id` 与基于证据包完成的 `completed_body`；脚本成功后设置 `recalled=true` 并给标题追加 `[召回补全内容]`。失败只读回执，不把失败状态写入记忆标题或正文。
