...
def _guide_option_action(option_id):
    return {
        ...
        "revise_task_plan": "按新证据修订当前工作计划结构。",
...
def _task_bootstrap_instruction_lines():
    return [
        "",
        "建账专用卡：",
        "- 初始清单是可修订工作计划：可先按用户请求、路径、URL 或文件名建立粗计划，不要在未知信息下假装已经读过材料。",
        "- 一次提交完整初始计划：source_refs=计划来源坐标，items=执行项，acceptance=验收项；source_requirements 与 risk_notes 均可选。",
        "- 最小结构：items=[{item_id, title}]；acceptance=[{acceptance_id, description, item_refs:[...]}]。需要精确来源映射时再加 source_requirements=[{requirement_id, source_ref, summary}] 与双方 requirement_refs。",
        "- reason/evidence_refs 属于每条 submission 外层，不得放进 fields；fields 只使用当前 option 的 allowed_fields。",
        "- source_refs 只是计划准备读取或核验的稳定坐标，不是读取成功证据；尚未返回的同帧工具结果也不得写成已知事实。",
        "- 每个 item_id 都必须至少被一条 acceptance.item_refs 覆盖；requirement_refs 不能替代 item_refs 的任务项覆盖。",
        ...
        "- 同一 response 的其他工具不会阻止建账；其结果要到结算后才能成为事实。",
        "- 清单很长就压缩描述，但 submit_initial_guide 仍必须带齐执行项和验收项；后续结构变化用 revise_task_plan，不要拿它登记进度。",
