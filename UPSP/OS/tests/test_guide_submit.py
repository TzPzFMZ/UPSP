...
def test_spec434_setup_popup_names_complex_task_guidance_trigger():
    ...
    assert "多步骤/多来源研究" in popup
    assert "执行命令、独立产物、验收或证据链交付" in popup
    assert "内部工具步骤不等于用户派发检索任务" in popup
    assert "单轮直接闭合的 `memory_write` 或 DC/EC/FUT" in popup
    assert "若这些沉淀只是更大任务的一步，仍按整个任务判 true" in popup
