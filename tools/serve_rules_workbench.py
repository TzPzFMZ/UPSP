#!/usr/bin/env python
"""Serve the UPSP rules visual workbench."""
...
from data.rules_workbench import (  # noqa: E402
...
def default_html_path() -> Path:
    return PROGRAM_OS_ROOT / "audit" / "rules_workbench.html"


CONFIG = WorkbenchServerConfig(
    name="rules workbench",
    log_prefix="rules-workbench",
    description="Serve UPSP rules visual workbench",
    html_route="/rules_workbench.html",
    api_route="/api/rules-workbench",
    files_route_prefix="/api/rules-workbench/files/",
    default_port=8767,
    default_repo_root=REPO_ROOT,
    default_html_path=default_html_path(),
    index_loader=build_workbench_index,
    document_loader=load_rule_document,
    diff_log_reader=read_diff_log,
    draft_saver=save_rule_draft,
    bad_path_error="bad_rule_path",
)
